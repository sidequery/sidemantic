...
The opt-in harness under `tests/malloy_differential/` compares the official Malloy DuckDB runtime with the Python Sidemantic path for manifest entries marked `compatible`. Currently executable fixtures cover core measures, empty results, and one typed arithmetic dimension. Other implemented families remain `planned` in the manifest until they receive a deterministic source, seed, Malloy query, native query, schema, and row oracle. A `planned` entry is not evidence that the Python adapter lacks the feature, and the documentation does not claim an official-runtime comparison where none exists.
