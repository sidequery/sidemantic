...
### Write Analyzable Queries

Good patterns for analysis:

✅ **Use explicit aliases**:
```sql
SELECT
    customer_id,
    SUM(amount) as total_revenue,  -- Uses "total_revenue"
    AVG(amount) as avg_order_value
FROM orders
...
SELECT status, region, COUNT(*)
FROM orders JOIN customers  -- Can't determine which table has status

-- Complex GROUP BY without matching SELECT
GROUP BY CASE WHEN ... THEN ... END  -- Hard to match to SELECT
```
