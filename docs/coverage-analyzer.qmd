...
The analyzer detects window function patterns and generates cumulative metric definitions.
...
**✅ Extracted as Cumulative Metrics**:
- `SUM/AVG/COUNT/MIN/MAX` with `OVER (ORDER BY ...)`
- Rolling windows with `ROWS/RANGE BETWEEN`
- Period-to-date with `PARTITION BY DATE_TRUNC(...)`

**❌ Not Extracted**:
- Window-only functions: `ROW_NUMBER()`, `RANK()`, `DENSE_RANK()`, `LAG()`, `LEAD()`
- Complex window calculations with operators (e.g., `COUNT(*) * 100.0 / SUM(COUNT(*)) OVER()`)

For complex calculations, the base aggregations are still extracted - you can define derived metrics on top of them.
