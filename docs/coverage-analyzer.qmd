...
```sql
SELECT sub.status, COUNT(*) as order_count
FROM (SELECT status, amount FROM orders WHERE amount > 100) sub
GROUP BY sub.status
```
...
```sql
SELECT order_date,
    SUM(amount) OVER (ORDER BY order_date ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) as rolling_7day
FROM orders
```
```yaml
metrics:
...
```sql
SELECT order_date,
    SUM(amount) OVER (PARTITION BY DATE_TRUNC('month', order_date) ORDER BY order_date) as mtd_revenue
FROM orders
```
```yaml
metrics:
