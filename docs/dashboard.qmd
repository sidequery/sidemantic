...
# SQL Editor {orientation="columns"}
...
```python {.marimo}
# Execute query (reactive to button and query changes)
SemanticLayer = import_module("sidemantic.core.semantic_layer").SemanticLayer

# Create semantic layer from existing graph
sql_layer = SemanticLayer()
sql_layer.graph = graph

# Reference button to create dependency
_ = run_button.value

try:
    result = sql_layer.sql(sql_query_input.value, conn=conn)
    ...
    mo.vstack([
        mo.md("**Compiled SQL:**"),
        mo.md(f"```sql\n{compiled_sql}\n```"),
        mo.md(f"**Results ({len(result_data)} rows):**"),
