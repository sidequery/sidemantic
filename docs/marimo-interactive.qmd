...
# Revenue over time stacked by category
data_cat_month, _ = run_query(metrics=["orders.revenue"], dims=["orders.order_date__month", "orders.category"], order_by=["orders.order_date__month"], _state=FILTERS_STATE)
chart_cat_month = (
    alt.Chart(alt.Data(values=data_cat_month))
    .mark_bar()
    .encode(
        x=alt.X("order_date__month:T", title="Month"),
        y=alt.Y("revenue:Q", title="Revenue"),
        color=alt.Color("category:N", title="Category"),
        tooltip=["order_date__month:T", "category:N", alt.Tooltip("revenue:Q", title="Revenue")],
    )
)
...
chart_mom = (
    alt.Chart(alt.Data(values=mom_data))
...
# Combine all charts into a single output
mo.vstack([
    mo.hstack([
        mo.vstack([mo.md('### Revenue by Month (Stacked by Status)'), mo.ui.altair_chart(chart_month)]),
        mo.vstack([mo.md('### Revenue by Month (Stacked by Category)'), mo.ui.altair_chart(chart_cat_month)]),
    ]),
    mo.md('<div style="height:16px"></div>'),
    mo.hstack([
        mo.vstack([mo.md('### Top Categories by Revenue'), mo.ui.altair_chart(chart_cat)]),
        mo.vstack([mo.md('### Revenue by Region'), mo.ui.altair_chart(chart_region)]),
    ]),
    mo.md('<div style="height:16px"></div>'),
...
Try editing the SQL model definition and querying it!

```python {.marimo}
# SQL Editor - Left: SQL model definitions, Right: SQL query editor
sql_model_def = mo.ui.code_editor(
    value="""MODEL (
    name orders,
    table orders,
    primary_key id
...
""",
    language="sql",
    label="Model Definition (SQL)"
)

sql_query_input = mo.ui.text_area(
    value="""SELECT revenue, order_count, status
FROM orders
...
mo.hstack([
    mo.vstack([
        mo.md("**Edit Model Definition:**"),
        sql_model_def,
    ], widths=[1]),
    mo.vstack([
        mo.md("**Write SQL Query:**"),
        sql_query_input,
    ], widths=[1]),
], widths=[1, 1])
```
...
```python {.marimo}
# Parse SQL definitions and execute query
from sidemantic.core.sql_definitions import parse_sql_model
from sidemantic.core.semantic_layer import SemanticLayer

try:
    # Parse the SQL model definition
    parsed_model = parse_sql_model(sql_model_def.value)

    if parsed_model:
        # Create semantic layer with the parsed model
        sql_editor_layer = SemanticLayer()
        sql_editor_layer.add_model(parsed_model)

        # Execute the SQL query
        result = sql_editor_layer.sql(sql_query_input.value, conn=conn)
        ...
        # Show compiled SQL and results
        mo.vstack([
            mo.md("**Compiled SQL:**"),
            mo.md(f"```sql\n{result.compiled_sql}\n```"),
            mo.md("**Results:**"),
            mo.ui.table(result_data) if result_data else mo.md("_No results_"),
        ])
    else:
        mo.md("⚠️ **Error:** Could not parse SQL model definition")
...
```sql
-- orders model
MODEL (name orders, table orders, primary_key id);
...
METRIC (name cost, agg sum, sql cost);
METRIC (name profit, agg sum, sql profit);
METRIC (name returns, agg count, filters status = 'returned');
METRIC (name return_rate, type ratio, numerator orders.returns, denominator orders.order_count);

SEGMENT (name completed, expression status = 'completed');
SEGMENT (name high_value, expression amount >= 300);
...
RELATIONSHIP (name region_map, type many_to_one, foreign_key region, primary_key region);

-- region_map model
MODEL (name region_map, table region_map, primary_key region);

DIMENSION (name region_group, type categorical, sql region_group);
```
...
mo.vstack([
    mo.md('### Semantic Model Definition (ER Diagram)'),
