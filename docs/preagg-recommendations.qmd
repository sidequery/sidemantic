...
Show summary statistics about query patterns.
...
Run 'sidemantic preagg recommend' to see recommendations
```

### preagg recommend

Show detailed recommendations with benefit scores.
...
```bash
# 1. Analyze 30 days of BigQuery history
sidemantic preagg analyze \
  --connection "bigquery://prod-project/analytics" \
  --days 30 \
  --min-count 100

# 2. Review top 10 recommendations
sidemantic preagg recommend \
  --connection "bigquery://prod-project/analytics" \
  --days 30 \
  --min-count 100 \
...
# 5. Refresh the new pre-aggregations
sidemantic refresh models/ --connection "bigquery://prod-project/analytics"
```
...
2. **Materialize tables**: Use `sidemantic refresh` to create physical tables
