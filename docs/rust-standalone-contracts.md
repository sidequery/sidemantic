...
| HTTP API | `sidemantic-server` with `runtime-server`; DB execution requires `runtime-server-adbc` | Rust-native JSON and Arrow IPC API for model metadata, graph discovery, semantic compile/query, semantic SQL rewrite/run, and raw select-only SQL run | Experimental, feature-gated; real server protocol tests cover startup failure, readiness/health, graph/models, compile/query/sql/raw routes, built-without-ADBC errors, DuckDB ADBC execution, buffered Arrow IPC response negotiation, and optional chunked Arrow transport streaming |
...
- `?format=arrow` or `Accept: application/vnd.apache.arrow.stream` returns a buffered Arrow IPC stream-format body with `Content-Type: application/vnd.apache.arrow.stream`, `X-Sidemantic-Row-Count`, `X-Sidemantic-Dialect`, and `X-Sidemantic-Arrow-Transport: buffered`.
- `?format=arrow&transport=chunked` streams the same Arrow IPC format over HTTP chunked transfer encoding. `?format=arrow&stream=true` is an alias. Chunked responses include `X-Sidemantic-Arrow-Transport: chunked` and omit `X-Sidemantic-Row-Count` because the server no longer buffers the full result to compute that header before response start.
