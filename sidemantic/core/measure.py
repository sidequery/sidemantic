...
class Measure(BaseModel):
    ...
    expr: str | None = Field(None, description="SQL expression or formula")

    # Metric type (if this is a complex metric, not just a simple aggregation)
    type: Literal["simple", "ratio", "derived", "cumulative", "time_comparison", "conversion"] | None = Field(
    ...
    @property
    def measure_ref(self) -> str | None:
        """For simple metrics, return the measure reference."""
        if self.type == "simple" and self.expr:
            # Simple metric that references a measure
            return self.expr
        return None
