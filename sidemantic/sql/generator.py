...
class SQLGenerator:
    ...
    def _custom_join_columns_by_model(self, base_model_name: str, other_models: list[str]) -> dict[str, set[str]]:
        ...
        query_models = frozenset({base_model_name, *other_models})
        for other_model in other_models:
            join_path = self.graph.find_relationship_path(base_model_name, other_model, query_models=query_models)
            if not join_path:
                continue
    ...
    def _participating_models(self, model_names: list[str]) -> set[str]:
        ...
        query_models = frozenset(model_names)
        for i, model_a in enumerate(names):
            for model_b in names[i + 1 :]:
                try:
                    join_path = self.graph.find_relationship_path(model_a, model_b, query_models=query_models)
                    for jp in join_path:
    ...
    def generate(
        ...
        all_models = set(model_names)
        query_model_set = frozenset(model_names)
        for i, model_a in enumerate(list(model_names)):
            for model_b in list(model_names)[i + 1 :]:
                # Find join path and add intermediate models
                try:
                    join_path = self.graph.find_relationship_path(model_a, model_b, query_models=query_model_set)
                    for jp in join_path:
                        all_models.add(jp.from_model)
                        all_models.add(jp.to_model)
                except ValueError:
    ...
    def _filter_references_metric(self, filter_expr: str, model_names: set[str]) -> bool:
        """Return True when a filter references a metric of any of the given models.

        Mirrors the metric detection in ``_classify_filters_for_pushdown`` so the
        pre-aggregation fanout path can separate HAVING-style metric filters (outer
        query, over projected metric columns) from dimension-level filters (pushed
        into every child sub-query). Unparseable filters are treated as
        metric-referencing so they stay on the outer query, where a wrong guess
        surfaces as a clear SQL error instead of silently changing populations.
        """
        try:
            parsed = _parse_fragment(filter_expr, self.dialect)
        except Exception:
    ...
    def _build_model_cte(
        ...
        # but only on the model whose measures get isolated (the fanout-safe plan
        # activates only when exactly one model owns every requested measure).
        # Projecting PKs on dimension-only models would reference columns the
        # physical table may not have (primary_key defaults to "id").
        owns_all_requested_metrics = False
        if len(all_models) > 1 and metrics:
            metric_owner_models: set[str] = set()
            for metric_ref in metrics:
                metric_owner_models.update(self._find_required_models([metric_ref], []))
            owns_all_requested_metrics = metric_owner_models == {model_name}
        include_primary_keys = (
            owns_all_requested_metrics or needs_keyed_joins or ungrouped or bool(model.sql) or bool(semi_additive)
        )
    ...
    def _needs_preaggregation_for_fanout(self, metrics: list[str], dimensions: list[str]) -> bool:
        ...
        # Path selection must see the same query context as the main compile path,
        # so include dimension models when tie-breaking diamond routes.
        dimension_models = {dim_ref.split(".", 1)[0] for dim_ref in dimensions if "." in dim_ref}
        query_models = frozenset(metric_models | dimension_models)
        ...
        for i, model_a in enumerate(metric_model_list):
            for model_b in metric_model_list[i + 1 :]:
                try:
                    ...
                    # Check reverse path
                    join_path_reverse = self.graph.find_relationship_path(model_b, model_a, query_models=query_models)
    ...
    def _generate_with_preaggregation(
        ...
        # Classify filters, then apply every dimension-level filter to EVERY child.
        # A query-level filter describes one population: partitioning filters by
        # owning model would filter one child's metrics but not its siblings',
        # producing a single result row whose metrics describe different
        # populations. Each child's recursive generate() joins whatever models a
        # filter references, and the fanout entity-isolation path keeps
        # filter-only joins from inflating the child's aggregates. Only
        # metric-referencing filters stay on the outer query (HAVING-style, over
        # the projected metric columns).
        ...
        child_filters: list[str] = [f for model_filters in pushdown_by_model.values() for f in model_filters]
        ...
        for filter_expr in shared_filters:
            if self._filter_references_metric(filter_expr, all_model_names):
                ...
                # Cross-model or non-metric-model dimension filter: the outer query
                # cannot evaluate it (its columns are not projected), but every
                # child can by joining the referenced models.
        ...
        for model_name, model_metrics in metrics_by_model.items():
            ...
            # Every child sees every dimension-level filter (one population),
            # plus any window-dim filters owned by this model. Window-dim filters
            # are pushed into the model's sub-query (not the outer preagg join)
            # so the recursive generate() handles them in its own outer WHERE,
            # preserving the requested dimension grain.
    ...
    def _build_main_select(
        ...
        query_models = frozenset(row_context_models | metric_models)
            ...
            for context_model in row_context_models:
                ...
                try:
                    join_path = self.graph.find_relationship_path(
                        metric_model, context_model, query_models=query_models
                    )
                except ValueError:
                    continue
...
"""
    ...
    def _join_key_preaggregation_route(
        ...
        for dim_ref, granularity in parsed_dims:
            ...
            try:
                query_models = frozenset(
                    {metric_model_name, *(ref.split(".", 1)[0] for ref, _ in parsed_dims if "." in ref)}
                )
                path = self.graph.find_relationship_path(metric_model_name, dim_model_name, query_models=query_models)
