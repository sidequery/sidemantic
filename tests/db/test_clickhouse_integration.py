...
def test_semantic_layer_basic_metric(clickhouse_layer):
    ...
    clickhouse_layer.add_model(orders)

    # Test sum
    result = clickhouse_layer.query(metrics=["orders.total_revenue"])
    row = result.fetchone()
    assert row[0] == 450.0

    # Test avg
    result = clickhouse_layer.query(metrics=["orders.avg_revenue"])
    row = result.fetchone()
    assert row[0] == 150.0

    # Test count
    result = clickhouse_layer.query(metrics=["orders.order_count"])
    row = result.fetchone()
    assert row[0] == 3


def test_semantic_layer_dimension_grouping(clickhouse_layer):
    ...
    clickhouse_layer.add_model(sales)

    # Group by one dimension
    result = clickhouse_layer.query(metrics=["sales.total_sales"], dimensions=["sales.category"])
...
def test_semantic_layer_joins(clickhouse_layer):
    ...
    clickhouse_layer.add_model(customers_join)
    clickhouse_layer.add_model(orders_join)

    # Query with join
    result = clickhouse_layer.query(metrics=["orders_join.total_revenue"], dimensions=["customers_join.region"])
    rows = result.fetchall()
    cols = [desc[0] for desc in result.description]
...
def test_semantic_layer_multiple_metrics(clickhouse_layer):
    ...
    clickhouse_layer.add_model(products)

    result = clickhouse_layer.query(
...
def test_semantic_layer_filters(clickhouse_layer):
    ...
    clickhouse_layer.add_model(inventory)
...
def test_semantic_layer_sql_generation(clickhouse_layer):
    ...
    clickhouse_layer.add_model(metrics_model)

    sql = clickhouse_layer.compile(metrics=["metrics_sql.total"])
    assert "SELECT" in sql.upper()
    assert "SUM" in sql.upper()
    assert clickhouse_layer.dialect == "clickhouse"


def test_semantic_layer_order_by(clickhouse_layer):
    ...
    clickhouse_layer.add_model(scores)
...
def test_semantic_layer_limit(clickhouse_layer):
    ...
    clickhouse_layer.add_model(items)

    result = clickhouse_layer.query(dimensions=["items.category"], metrics=["items.count"], limit=2)
    rows = result.fetchall()
    assert len(rows) == 2


@pytest.mark.skip(reason="Date functions test - consistent with BigQuery/Postgres/Snowflake")
def test_semantic_layer_date_functions(clickhouse_layer):
    """Test date/time functions in metrics."""
    events = Model(
        name="events",
        table="""(
            SELECT toDate('2024-01-15') as event_date, 1 as event_id UNION ALL
            SELECT toDate('2024-01-20'), 2 UNION ALL
            SELECT toDate('2024-02-10'), 3 UNION ALL
            SELECT toDate('2024-02-15'), 4
        )""",
        primary_key="event_id",
        dimensions=[
            Dimension(name="month", type="time", sql="formatDateTime(event_date, '%Y-%m')", granularity="month"),
            Dimension(name="year", type="time", sql="formatDateTime(event_date, '%Y')", granularity="year"),
        ],
        metrics=[Metric(name="event_count", agg="count", sql="event_id")],
    )
    clickhouse_layer.add_model(events)

    result = clickhouse_layer.query(dimensions=["events.month"], metrics=["events.event_count"])
    rows = result.fetchall()
...
def test_semantic_layer_symmetric_aggregates(clickhouse_layer):
    ...
    clickhouse_layer.add_model(orders_sym)
    clickhouse_layer.add_model(line_items_sym)

    # Query both metrics - should use symmetric aggregation to avoid fan-out
    result = clickhouse_layer.query(metrics=["orders_sym.total_subtotal", "line_items_sym.total_price"])
    row = result.fetchone()
    cols = [desc[0] for desc in result.description]
    row_dict = dict(zip(cols, row))
...
def test_semantic_layer_multiple_joins(clickhouse_layer):
    ...
    clickhouse_layer.add_model(users_multi)
    clickhouse_layer.add_model(products_multi)
    clickhouse_layer.add_model(orders_multi)
