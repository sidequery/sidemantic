"""Integration tests for Snowflake adapter using fakesnow.

Run with: SNOWFLAKE_TEST=1 pytest -m integration tests/db/test_snowflake_integration.py -v
"""
...
def snowflake_layer():
    """Create SemanticLayer with Snowflake connection."""
    # Use a dummy connection URL - fakesnow will intercept the connection
    layer = SemanticLayer(connection="snowflake://test:test@test/testdb/public?warehouse=test_warehouse")
    yield layer
    layer.adapter.close()


def test_semantic_layer_basic_metric(snowflake_layer):
    ...
    snowflake_layer.add_model(orders)

    # Test sum
    result = snowflake_layer.query(metrics=["orders.total_revenue"])
    row = result.fetchone()
    assert row[0] == 450.0

    # Test avg
    result = snowflake_layer.query(metrics=["orders.avg_revenue"])
    row = result.fetchone()
    assert row[0] == 150.0

    # Test count
    result = snowflake_layer.query(metrics=["orders.order_count"])
    row = result.fetchone()
    assert row[0] == 3


def test_semantic_layer_dimension_grouping(snowflake_layer):
    ...
    snowflake_layer.add_model(sales)

    # Group by one dimension
    result = snowflake_layer.query(metrics=["sales.total_sales"], dimensions=["sales.category"])
...
def test_semantic_layer_joins(snowflake_layer):
    ...
    snowflake_layer.add_model(customers_join)
    snowflake_layer.add_model(orders_join)

    # Query with join
    result = snowflake_layer.query(metrics=["orders_join.total_revenue"], dimensions=["customers_join.region"])
    rows = result.fetchall()
    cols = [desc[0] for desc in result.description]
    # Snowflake returns uppercase column names
...
def test_semantic_layer_multiple_metrics(snowflake_layer):
    ...
    snowflake_layer.add_model(products)

    result = snowflake_layer.query(
...
def test_semantic_layer_filters(snowflake_layer):
    ...
    snowflake_layer.add_model(inventory)
...
def test_semantic_layer_sql_generation(snowflake_layer):
    ...
    snowflake_layer.add_model(metrics_model)

    sql = snowflake_layer.compile(metrics=["metrics_sql.total"])
    assert "SELECT" in sql.upper()
    assert "SUM" in sql.upper()
    assert snowflake_layer.dialect == "snowflake"


def test_semantic_layer_order_by(snowflake_layer):
    ...
    snowflake_layer.add_model(scores)
...
def test_semantic_layer_limit(snowflake_layer):
    ...
    snowflake_layer.add_model(items)

    result = snowflake_layer.query(dimensions=["items.category"], metrics=["items.count"], limit=2)
    rows = result.fetchall()
    assert len(rows) == 2


@pytest.mark.skip(reason="Date functions test - consistent with BigQuery/Postgres")
def test_semantic_layer_date_functions(snowflake_layer):
    """Test date/time functions in metrics."""
    events = Model(
        name="events",
        table="""(
            SELECT TO_DATE('2024-01-15') as event_date, 1 as event_id UNION ALL
            SELECT TO_DATE('2024-01-20'), 2 UNION ALL
            SELECT TO_DATE('2024-02-10'), 3 UNION ALL
            SELECT TO_DATE('2024-02-15'), 4
        )""",
        primary_key="event_id",
        dimensions=[
            Dimension(name="month", type="time", sql="TO_CHAR(event_date, 'YYYY-MM')", granularity="month"),
            Dimension(name="year", type="time", sql="TO_CHAR(event_date, 'YYYY')", granularity="year"),
        ],
        metrics=[Metric(name="event_count", agg="count", sql="event_id")],
    )
    snowflake_layer.add_model(events)

    result = snowflake_layer.query(dimensions=["events.month"], metrics=["events.event_count"])
    rows = result.fetchall()
...
def test_semantic_layer_symmetric_aggregates(snowflake_layer):
    ...
    snowflake_layer.add_model(orders_sym)
    snowflake_layer.add_model(line_items_sym)

    # Query both metrics - should use symmetric aggregation to avoid fan-out
    result = snowflake_layer.query(metrics=["orders_sym.total_subtotal", "line_items_sym.total_price"])
    row = result.fetchone()
    cols = [desc[0] for desc in result.description]
    row_dict = dict(zip(cols, row))
...
def test_semantic_layer_multiple_joins(snowflake_layer):
    ...
    snowflake_layer.add_model(users_multi)
    snowflake_layer.add_model(products_multi)
    snowflake_layer.add_model(orders_multi)
