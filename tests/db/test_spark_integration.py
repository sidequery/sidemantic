"""Integration tests for Spark adapter.

Run with: docker compose up -d spark && pytest -m integration tests/db/test_spark_integration.py -v
"""
...
def spark_layer():
    """Create SemanticLayer with Spark connection."""
    layer = SemanticLayer(connection=SPARK_URL)
    yield layer
    layer.adapter.close()


def test_semantic_layer_basic_metric(spark_layer):
    ...
    spark_layer.add_model(orders)

    # Test sum
    result = spark_layer.query(metrics=["orders.total_revenue"])
    row = result.fetchone()
    assert row[0] == 450.0

    # Test avg
    result = spark_layer.query(metrics=["orders.avg_revenue"])
    row = result.fetchone()
    assert row[0] == 150.0

    # Test count
    result = spark_layer.query(metrics=["orders.order_count"])
    row = result.fetchone()
    assert row[0] == 3


def test_semantic_layer_dimension_grouping(spark_layer):
    ...
    spark_layer.add_model(sales)

    # Group by one dimension
    result = spark_layer.query(metrics=["sales.total_sales"], dimensions=["sales.category"])
...
def test_semantic_layer_joins(spark_layer):
    ...
    spark_layer.add_model(customers_join)
    spark_layer.add_model(orders_join)

    # Query with join
    result = spark_layer.query(metrics=["orders_join.total_revenue"], dimensions=["customers_join.region"])
    rows = result.fetchall()
    cols = [desc[0] for desc in result.description]
...
def test_semantic_layer_multiple_metrics(spark_layer):
    ...
    spark_layer.add_model(products)
...
def test_semantic_layer_filters(spark_layer):
    ...
    spark_layer.add_model(inventory)
...
def test_semantic_layer_sql_generation(spark_layer):
    ...
    spark_layer.add_model(metrics_model)

    sql = spark_layer.compile(metrics=["metrics_sql.total"])
    assert "SELECT" in sql.upper()
    assert "SUM" in sql.upper()
    assert spark_layer.dialect == "spark"


def test_semantic_layer_order_by(spark_layer):
...
def test_semantic_layer_limit(spark_layer):
    ...
    spark_layer.add_model(items)

    result = spark_layer.query(dimensions=["items.category"], metrics=["items.count"], limit=2)
    rows = result.fetchall()
    assert len(rows) == 2


@pytest.mark.skip(reason="Date functions test - consistent with BigQuery/Postgres/Snowflake")
def test_semantic_layer_date_functions(spark_layer):
    ...
    spark_layer.add_model(events)

    result = spark_layer.query(dimensions=["events.month"], metrics=["events.event_count"])
    rows = result.fetchall()
...
def test_semantic_layer_symmetric_aggregates(spark_layer):
    ...
    spark_layer.add_model(orders_sym)
    spark_layer.add_model(line_items_sym)

    # Query both metrics - should use symmetric aggregation to avoid fan-out
    result = spark_layer.query(metrics=["orders_sym.total_subtotal", "line_items_sym.total_price"])
    row = result.fetchone()
    cols = [desc[0] for desc in result.description]
    row_dict = dict(zip(cols, row))
...
def test_semantic_layer_multiple_joins(spark_layer):
    ...
    spark_layer.add_model(users_multi)
    spark_layer.add_model(products_multi)
    spark_layer.add_model(orders_multi)
