...
def test_three_way_preagg_joins_later_ctes_on_coalesced_dimension_key():
    ...
    The orders filter removes the pending date from the first pre-aggregation,
    while both child-model pre-aggregations still contain it. The items and
    shipments values for that date must land on one row, not two split rows.
    """
    conn = duckdb.connect(":memory:")
    conn.execute("""
        CREATE TABLE raw_orders AS
        SELECT * FROM (VALUES
            (1, DATE '2024-01-01', 100, 'shipped'),
            (2, DATE '2024-01-02', 200, 'pending')
        ) AS t(id, order_date, amount, status)
    """)
    conn.execute("""
        CREATE TABLE raw_items AS
        SELECT * FROM (VALUES (1, 1, 3), (2, 2, 7)) AS t(id, order_id, quantity)
    """)
    conn.execute("""
        CREATE TABLE raw_shipments AS
    ...
    sql = SQLGenerator(graph).generate(
        metrics=["orders.revenue", "items.quantity", "shipments.shipment_count"],
        dimensions=["orders.order_date"],
        filters=["orders.status = 'shipped'"],
    ...
    assert len(rows) == 2
    pending_date = next(row for row in rows if str(row[0]) == "2024-01-02")
    assert pending_date[1] is None
    assert pending_date[2:] == (7, 2)
