...
def test_secure_information_schema_columns_uses_visible_semantic_catalog():
    ...
    conn._handle_query(
        "SELECT column_name, udt_name, character_octet_length, numeric_precision_radix "
        "FROM information_schema.columns "
        "WHERE table_schema = 'semantic_layer' AND table_name = 'orders' ORDER BY ordinal_position;",
        lambda *_: None,
    )
    catalog_rows = captured["reader"].read_all().to_pylist()
    assert catalog_rows == [
        {
            "column_name": "status",
            "udt_name": "varchar",
    ...
    conn._handle_query("SELECT nspname FROM pg_catalog.pg_namespace;", lambda *_: None)
    assert [row["nspname"] for row in captured["reader"].read_all().to_pylist()] == ["semantic_layer"]
