...
def three_table_chain():
    ...
    conn.execute(
        """CREATE TABLE orders (order_id INTEGER, customer_id INTEGER, order_amount DECIMAL(10, 2))"""
    )
    conn.execute(
        """CREATE TABLE customers (customer_id INTEGER, region_id INTEGER, customer_name VARCHAR)"""
    )
    conn.execute("""CREATE TABLE regions (region_id INTEGER, region_name VARCHAR)""")

    conn.execute(
        """INSERT INTO orders VALUES (1, 101, 150.00), (2, 102, 200.00), (3, 101, 100.00), (4, 103, 300.00)"""
    )
    conn.execute(
        """INSERT INTO customers VALUES (101, 1, 'Alice'), (102, 2, 'Bob'), (103, 1, 'Charlie')"""
    )
