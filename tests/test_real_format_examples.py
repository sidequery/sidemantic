...
def test_import_real_gooddata_example():
    """Test importing real GoodData dataset files."""
    adapter = GoodDataAdapter()
    graph = adapter.parse("examples/gooddata/")
    ...
    orders = graph.models["orders"]
    assert orders.table == "public/orders"
    assert orders.description == "Order transactions with customer and product information"
    assert orders.primary_key == "order_id"

    # Verify fields (attributes and facts)
    dim_names = [d.name for d in orders.dimensions]
    assert "order_id" in dim_names
    assert "customer_id" in dim_names
    assert "order_date" in dim_names
    assert "status" in dim_names
    assert "amount" in dim_names  # fact as numeric dimension
    assert "quantity" in dim_names  # fact as numeric dimension

    # Verify fact is numeric dimension
    amount = next(d for d in orders.dimensions if d.name == "amount")
    assert amount.type == "numeric"

    # Verify attribute is categorical dimension
    status = next(d for d in orders.dimensions if d.name == "status")
    assert status.type == "categorical"

    # Verify date attribute is time dimension
...
def test_import_gooddata_with_labels():
    """Test that GoodData attribute labels are imported as dimensions."""
    adapter = GoodDataAdapter()
    graph = adapter.parse("examples/gooddata/orders.yaml")

    orders = graph.models["orders"]

    # Verify label dimension created
    dim_names = [d.name for d in orders.dimensions]
    assert "status_label" in dim_names


def test_gooddata_to_sidemantic_to_gooddata_roundtrip():
    """Test roundtrip: GoodData → Sidemantic → GoodData."""
    adapter = GoodDataAdapter()

    # Import original
    graph1 = adapter.parse("examples/gooddata/")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        orders1 = graph1.models["orders"]
        orders2 = graph2.models["orders"]
        # Note: metrics are stored separately in GoodData, so we only check dimensions
        assert len(orders1.dimensions) == len(orders2.dimensions)


def test_gooddata_to_cube_conversion():
    """Test converting GoodData dataset to Cube format."""
    gooddata_adapter = GoodDataAdapter()
    cube_adapter = CubeAdapter()

    # Import from GoodData
    graph = gooddata_adapter.parse("examples/gooddata/")
...
def test_cube_to_gooddata_conversion():
    """Test converting Cube schema to GoodData dataset."""
    cube_adapter = CubeAdapter()
    gooddata_adapter = GoodDataAdapter()
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Import GoodData version
        gooddata_graph = gooddata_adapter.parse(output_path)

        # Verify model exists
        assert "orders" in gooddata_graph.models


def test_gooddata_to_metricflow_conversion():
    """Test converting GoodData dataset to MetricFlow."""
    gooddata_adapter = GoodDataAdapter()
    mf_adapter = MetricFlowAdapter()
...
def test_gooddata_to_superset_conversion():
    """Test converting GoodData dataset to Superset."""
    gooddata_adapter = GoodDataAdapter()
    superset_adapter = SupersetAdapter()
...
def test_gooddata_to_omni_conversion():
    """Test converting GoodData dataset to Omni."""
    gooddata_adapter = GoodDataAdapter()
    omni_adapter = OmniAdapter()
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (output_path / "views" / "orders.yaml").exists()
