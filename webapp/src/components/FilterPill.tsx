...
export function FilterPill(props: FilterPillProps) {
  ...
  if (!("dim" in props)) {
    return (
      <span data-dimension={props.dimension} data-value={props.value} className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-line bg-surface px-2 py-0.5 text-2xs text-muted">
  ...
  return (
    <span className="relative inline-flex max-w-full items-center" data-dimension={dim.ref} data-mode={filter.mode}>
      <span className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-line bg-surface px-2 py-0.5 text-2xs text-muted">
        <button
          type="button"
