...
export function FilterPill(props: FilterPillProps) {
  ...
  if (!("dim" in props)) {
    return (
      <span data-dimension={props.dimension} data-value={props.value} className="inline-flex max-w-full items-center gap-1 rounded-full bg-surface-soft px-2 py-0.5 text-2xs leading-4 text-muted">
        <span className="truncate"><span className="text-faint">{props.dimensionLabel ?? props.dimension}:</span> {props.value}</span>
        {props.onRemove ? (
          <button type="button" aria-label={`Remove filter ${props.value}`} onClick={props.onRemove} className="-mr-0.5 px-0.5 text-faint hover:text-ink">×</button>
        ) : null}
      </span>
    );
  }
  ...
  return (
    <span className="relative inline-flex max-w-full items-center" data-dimension={dim.ref} data-mode={filter.mode}>
      <span className="inline-flex max-w-full items-center gap-1 rounded-full bg-surface-soft px-2 py-0.5 text-2xs leading-4 text-muted">
        <button
          type="button"
