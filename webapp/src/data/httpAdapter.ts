...
export class HttpBackend implements SidemanticBackend {
  ...
  async getDashboard(): Promise<DashboardSpec | null> {
    try {
      ...
      const payload = (await res.json()) as Partial<DashboardSpec>;
      return typeof payload.title === "string" && Array.isArray(payload.tabs) ? (payload as DashboardSpec) : null;
    } catch {
  ...
  async createBrowserSession(token: string): Promise<void> {
    ...
    if (res.status === 404 || (res.ok && !res.headers.get("content-type")?.includes("application/json"))) {
      // Backends without the exchange endpoint keep the bearer only in this
      // adapter instance until the page is reloaded.
      this.token = token;
      return;
    }
