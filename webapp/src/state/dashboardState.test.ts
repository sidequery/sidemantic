...
describe("dashboard selections", () => {
  ...
  test("preserves explicit scatter and point chart types", () => {
    const types = { "orders.amount": "numeric" };
    const chart: DashboardChart = {
      id: "amounts",
      type: "auto",
      query: { metrics: ["orders.revenue"], dimensions: ["orders.amount"] },
      encoding: { x: "orders.amount", y: "orders.revenue" },
    };

    expect(dashboardChartType({ ...chart, type: "scatter" }, types)).toBe("scatter");
    expect(dashboardChartType({ ...chart, type: "point" }, types)).toBe("point");
  });
