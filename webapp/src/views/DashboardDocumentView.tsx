...
function DashboardChartPanel({
  ...
  if (query.loading && !query.result) {
    ...
    const colorColumn = chart.encoding?.color
      ? dashboardResultColumn(chart.encoding.color, columns)
      : seriesColumns[0];
    visualization = (
      <div className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-3">
        {yRefs.map((metric) => {
          ...
          const points = rows.map((row) => ({
            ...
            series: colorColumn ? String(row[colorColumn] ?? "") : undefined,
          }));
          return (
            <div key={metric} className="min-w-0">
              ...
              <ScatterChart
                points={points}
                xLabel={labelize(xRef)}
                yLabel={labelize(metric)}
                formatY={(value) => formatValue(value, metricFormat(catalog, metric))}
                ariaLabel={`${chartTitle}, ${labelize(metric)} by ${labelize(xRef)}, ${points.length} points`}
              />
            </div>
          );
        })}
